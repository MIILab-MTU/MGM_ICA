...
class Siamese_SelfAttention(nn.Module):
    r"""
    Siamese SelfAttention neural network for processing arbitrary number of graphs.

    :param in_features: the dimension of input node features
    :param num_features: the dimension of output node features
    """
    def __init__(self, in_features, num_features):
        super(Siamese_SelfAttention, self).__init__()
        self.self_attention = SelfAttention(in_features, num_features)

    def forward(self, g1: Tuple[Tensor, Tensor, Tensor, int], *args) -> Union[Tensor, List[Tensor]]:
        r"""
        Forward computation of Siamese SelfAttention.
        ...
        """
        # embx are tensors of size (bs, N, num_features)
        emb1 = self.self_attention(*g1)
        if len(args) == 0:
            return emb1
        else:
            returns = [emb1]
            for g in args:
                returns.append(self.self_attention(*g))
            return returns

class SelfAttention(nn.Module):
    r"""
    Self Attention Module for graph nodes.
    ...
    def forward(self, A: Tensor, x: Tensor, norm: bool=True) -> Tensor:
        r"""
        Forward computation of self-attention network.
        A: batch*n_node*n_node;
        x: batch*n_node*node_feature_dim
        ...
        attn_weights = F.softmax(torch.bmm(ax, ax.transpose(1, 2)), dim=-1)
        attended_x = torch.bmm(attn_weights, ax)

        x = F.relu(ux) + attended_x  # has size (bs, N, num_outputs)
        return x